...
let anteriorButton = getElement("#anterior");
let proximoButton = getElement("#proximo");

anteriorButton.addEventListener('click', () => changeImage(-1));
proximoButton.addEventListener('click', () => changeImage(1));

function changeImage(offset) {
    function getNextImageIndex(offset) {
        /* Dado que:
            (index + length) % length = index
            (offset % length), para length de 8, retorna um valor entre -7 e 7.
           Podemos juntar os dois abaixo, para evitar qualquer comparação.
        */
        const length = imagens.length;
        let nextIndex = (currentImageIndex + length + offset % length) % length
        return nextIndex
    }
    function getImageLink(fileUrl) {
        return servidorDasImagens + "/" + fileUrl
    }

    currentImageIndex = getNextImageIndex(offset);

    let slideElement = getElement('#slide')
    slideElement.alt = imagens[currentImageIndex].descricao;
    slideElement.src = getImageLink(imagens[currentImageIndex].arquivo);
}
